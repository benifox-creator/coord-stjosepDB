...
1. ~~Tancar el forat de seguretat d'`incidencies.reporter`~~ — **fet i verificat** amb test de regressió.
2. ~~Decidir què fer amb els canvis sense commitejar~~ — **fet**. Confirmat amb la sessió "Dispatch background conversation": treball seu, deliberadament deixat sense commitejar per a revisió, sense cap procés en marxa ni intenció d'escriure-hi més. Ens cedeix la coordinació del worktree. Organitzat i commitejat en 7 commits lògics (autenticació/client de dades, migracions 001-004, migracions 005-007 + Edge Function + fix de seguretat, refactor de rutes, suite de tests, mòduls del client, documentació/CI). Working tree net; `npm test` (59/59), `npm run build` i `npm run lint` sense errors sobre l'estat final commitejat.
...
4. ~~Afegir els tests de RLS que falten~~ — **fet**: aïllament d'absències entre professors, `substitution_read`, `own_notifications` (incloent que un `manager` no admin tampoc hi té accés), denegació d'escriptura a `convidat`, i `app_private.minutes` amb entrada malformada. Suite: 59/59.
5. **Verificació manual final amb sessions reals de cada rol** (Tasca 10 original, mai feta) — necessita algú amb accés real al domini `@stjosep.org`. Únic pas que queda genuïnament pendent.
6. ~~Decidir l'abast d'aquest merge~~ — resolt: la sessió "Dispatch background conversation" cedeix explícitament la coordinació d'aquest worktree ("considera mi intervención cerrada"). Es manté tot sota `feat-horaris`; no cal separar-ho en una branca pròpia.

**Important, en paraules de qui ho va escriure:** "Las 53 pruebas que mencioné correspondían al estado que dejé; no certificaban la ausencia de otros fallos de seguridad." Aquesta revisió n'ha afegit 6 més (1 arreglant un forat real, 5 de cobertura) i n'ha revisat el nucli a fons, però amb un abast d'aquesta mida (auditoria, préstecs, reserves, notificacions...) cap revisió puntual hauria de considerar-se l'última paraula abans de tocar dades reals del centre.
